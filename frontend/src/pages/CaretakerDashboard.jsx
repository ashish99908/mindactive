...
const CaretakerDashboard = () => {
  ...
  const handleReportChange = (e) => {
    ...
    if (!okTypes.includes(file.type)) { setReportFile(null); setReportError('Only PDF, PNG or JPG files are allowed'); return; }
    if (file.size > 5 * 1024 * 1024) { setReportFile(null); setReportError('File too large — maximum size is 5 MB'); return; }
    setReportFile(file);
  };
    ...
    try {
      ...
      if (reportFile && res.data?.patientId) {
        try {
          ...
          reportNote = ' Medical report uploaded.';
        } catch (upErr) {
          reportNote = ` (Report upload failed: ${upErr.response?.data?.error || 'network error'} — you can attach it later from the patient page.)`;
        }
      }
      setFormSuccess(`Patient added successfully!${reportNote}`);
  ...
  const metrics = [
    { icon: '👥', label: 'Total Patients', value: totalPatients, tint: 'tint-purple' },
    { icon: '🎮', label: 'Games Completed', value: totalGames, tint: 'tint-teal' },
    { icon: '📊', label: 'Avg Accuracy', value: `${Math.round(avgAccuracy)}%`, tint: 'tint-amber' },
    { icon: '🟢', label: 'Active Patients', value: activePatients, tint: 'tint-green' },
  ];
  ...
  return (
    <div className="container page-pad">
      ...
      <div className="dash-hero fade-up">
        <div>
          <h1 className="page-title" style={{ color: '#fff' }}>Welcome back, {user?.name?.split(' ')[0]} 👋</h1>
      ...
      {/* Add Patient Form */}
      {showAddForm && (
        <div className="card fade-up" style={{ border: '1.5px solid var(--c-primary-soft)' }}>
          ...
          <form onSubmit={handleAddPatient}>
            ...
            <div>
              <label>Medical report (optional — PDF, JPG or PNG, max 5 MB)</label>
              <label className="file-drop">
                ...
                <span>{reportFile ? `📄 ${reportFile.name} (${Math.max(1, Math.round(reportFile.size / 1024))} KB) — click to change` : '📎 Attach medical report (PDF or image)'}</span>
              </label>
              {reportError && <div className="alert alert-error">{reportError}</div>}
            </div>

            <label>Notes (optional)</label>
            <textarea name="notes" value={formData.notes} onChange={handleInputChange} rows="2" placeholder="Anything important about this patient…" />
            {formError && <div className="alert alert-error">{formError}</div>}
            {formSuccess && <div className="alert alert-success">{formSuccess}</div>}
            <button type="submit" style={{ width: '100%' }}>Create Patient ✓</button>
          </form>
        </div>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, flexWrap: 'wrap', gap: 8 }}>
        <h2 className="section-title" style={{ marginBottom: 0 }}>Your patients</h2>
        <span className="badge badge-gray">{patients.length} {patients.length === 1 ? 'patient' : 'patients'}</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>Click “Add Patient” above to create your first patient profile.</p>
        </div>
      ) : (
        <div className="grid">
          {patients.map(patient => (
            <div key={patient.patient_id} className="card hoverable" style={{ marginBottom: 0, display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {patient.age && <div style={{ fontSize: '.82rem', color: 'var(--c-muted)' }}>🎂 {patient.age} yrs{patient.preferred_language ? ` · 🗣️ ${patient.preferred_language}` : ''}</div>}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                <span className="badge badge-green">{patient.games_completed || 0} games</span>
                <span className="badge badge-blue">{Math.round(patient.avg_accuracy || 0)}% acc</span>
                {patient.last_activity
                  ? <span className="badge badge-gray">Last: {new Date(patient.last_activity).toLocaleDateString()}</span>
                  : <span className="badge badge-amber">No activity yet</span>}
              </div>
