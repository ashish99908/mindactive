...
export default function Analytics() {
  ...
  const buckets = [
    { name: 'Excellent (80%+)', min: 80, max: 101, color: '#22c55e' },
    { name: 'Good (60–79%)', min: 60, max: 80, color: '#14b8a6' },
    { name: 'Needs focus (<60%)', min: -1, max: 60, color: '#f59e0b' },
  ].map(b => ({
    name: b.name,
  ...
  const statTiles = [
    { icon: '👥', label: 'Patients', value: patients.length, tint: 'tint-purple' },
    { icon: '🎮', label: 'Total Sessions', value: totalGames, tint: 'tint-teal' },
    { icon: '📊', label: 'Cohort Accuracy', value: `${Math.round(avgAccuracy)}%`, tint: 'tint-amber' },
    { icon: '🟢', label: 'Active (7d)', value: activePatients, tint: 'tint-green' },
  ];
  ...
  return (
    <div className="container page-pad">
      <div className="fade-up" style={{ marginBottom: 26 }}>
        ...
        <p className="page-subtitle">A bird's-eye view of how your whole group is doing.</p>
      </div>
      ...
      {patients.length === 0 ? (
        <div className="empty-state fade-up-1">
          ...
          <p>Add patients and let them play a few games — insights will appear here.</p>
        ...
        <>
          ...
          <div className="grid-2 fade-up-2">
            <div className="card" style={{ marginBottom: 0 }}>
              <div className="card-title">🎮 Engagement — sessions per patient</div>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={engagementData} margin={{ top: 5, right: 10, left: -18, bottom: 5 }}>
                  ...
                  <Tooltip />
                  <Bar dataKey="games" name="Sessions" radius={[8, 8, 0, 0]}>
            ...
            <div className="card" style={{ marginBottom: 0 }}>
              <div className="card-title">🎯 Accuracy — performance bands</div>
              {buckets.length === 0 ? (
                <p className="text-muted">No accuracy data yet.</p>
              ) : (
                <ResponsiveContainer width="100%" height={280}>
                  <PieChart>
            ...
            <div className="card" style={{ marginBottom: 0, gridColumn: '1 / -1' }}>
              <div className="card-title">📈 Average accuracy per patient</div>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={accuracyData} margin={{ top: 5, right: 10, left: -18, bottom: 5 }}>
                  ...
                  <Tooltip />
                  <Bar dataKey="accuracy" name="Avg accuracy %" radius={[8, 8, 0, 0]} fill="#6d5ef2" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid-2 fade-up-3" style={{ marginTop: 22 }}>
            {topPerformer && (
              <div className="card" style={{ marginBottom: 0, background: 'var(--c-green-soft)', border: '1px solid #bbf7d0' }}>
                <div className="card-title">🌟 Star of the week</div>
                <p className="text-body">
                  <strong>{topPerformer.name}</strong> leads the group with{' '}
                  <strong>{Math.round(topPerformer.avg_accuracy || 0)}% average accuracy</strong> across{' '}
                  {topPerformer.games_completed || 0} sessions. Wonderful consistency!
                </p>
                <Link to={`/caretaker/patient/${topPerformer.patient_id}`}>
                  <button className="small" style={{ marginTop: 12 }}>View {topPerformer.name?.split(' ')[0]}'s details →</button>
                </Link>
              </div>
            )}
            <div className="card" style={{ marginBottom: 0, background: 'var(--c-accent-soft)', border: '1px solid #fde68a' }}>
              <div className="card-title">🔔 May need attention</div>
              {needsAttention.length === 0 ? (
                <p className="text-body">Everyone is doing great — no one needs special attention right now. 🎉</p>
              ) : (
                  ...
                  {needsAttention.slice(0, 4).map(p => (
                    <div key={p.patient_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                      ...
                      <span className={`badge ${!p.last_activity ? 'badge-amber' : 'badge-red'}`}>
                        {!p.last_activity ? 'Inactive' : `${Math.round(p.avg_accuracy || 0)}% acc`}
                      </span>
                    </div>
                  ))}
                </div>
